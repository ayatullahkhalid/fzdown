...
export default class Scraper {
  ...
  fetchUrl = async (url, kind = this.movies, retry = 3) => {
    const cookieJar = kind == this.movies ? this.movieCookieJar : this.seriesCookieJar
    ...
    for (let i = 0; i < retry; i++) {
      try {
        ...
        if (!res.ok) {
          ...
          console.log("STATUS:", await res.status, "URL:", url);
  ...
  parseSeries = async ($, count = 20) => {
    const shows = [];
    $('.mainbox3 > table').parent('.mainbox3').slice(0, count).each((i, el) => {
      const title = $(el).find("span a b").first().text().trim();
  ...
  parseMovies = async ($, count = 5) => {
    const movies = [];
    $('.mainbox > table').parent('.mainbox').slice(0, count).each((_, el) => {
      ...
      const desc = tagDesc.contents().filter((_, node) => node.type === "text").text().trim();
      const tags = [];
  ...
  scrapeSeries = async () => {
    ...
    const html = await this.fetchUrl(this.makeUrl("/trending.php", this.baseSeriesURL));
  ...
  scrapeMovies = async () => {
    ...
    const html = await this.fetchUrl(this.makeUrl("/movieslist.php", this.baseMoviesURL));
    const $ = cheerio.load(html);
    return this.parseMovies($);
  };

  search = async (query, kind = this.series) => {
    const baseURL = kind === this.series ? this.baseSeriesURL : this.baseMoviesURL;
    ...
    return kind === this.series ? this.parseSeries($, 20) : this.parseMovies($, 20);
  ...
  getMovie = async (url) => {
    ...
    const dlinks = await Promise.all(
      files.map(file => this.getMovieFiles(file.link, referer).catch(() => null))
  ...
  getShow = async (url) => {
    ...
    const genres = textNodes[2].replace(':', '')?.split(',')?.map(g => g.trim())
    const rating =  textNodes[3].replace(':', '').trim()
  
    const linkBox = $("div")
    $('[itemprop="containsSeason"] .mainbox2 a').each((_, el) => {
      seasons.push($(el).attr("href"))
      sTitles.push($(el).text())
    })

     return {title, desc, running, genres, seasons, sTitles};
  }
  ...
  getEpisodeList = async (url) => {
    ...
    $('.mainbox > table').parent('.mainbox').each((_, el) => {
      const title = $(el).find("span b").text().replace("_"," ").trim()
      const link = this.makeUrl($(el).find("span a").first().attr("href"), this.baseSeriesURL)
      const airing = $(el).find("span i").first().text().split(":")[1].replace(/[(),:_]/g, '').trim()
      const desc = $(el).find("small").last().contents().filter((i, el) => el.type === 'text' && $(el).text().trim()).first().text().trim()
      const $img = $(el).find("img");
      const img = $img.length ? $img.attr("src") : null;
  ...
  getEpisodeMp4 = async (url, referer) => {
    ...
    html = await this.fetchUrl(this.makeUrl(downloadLink, this.baseSeriesURL), this.series)
    $ = cheerio.load(html)
  ...
  getEpisodes = async (episodes, referer, start = 0, end = 5) => {
    const slicedEpisodes = episodes.slice(start, end);
    const results = await Promise.all(
      slicedEpisodes.map(async (episode) => {
        const [mp4, webm] = await Promise.allSettled([
        ...
        return {
          ...episode,
          mp4: mp4.status === 'fulfilled' ? mp4.value : null,
          webm: webm.status === 'fulfilled' ? webm.value : null
        };
      })
    );
    console.log(results)
