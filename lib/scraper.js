...
export default class Scraper {
  ...
  getEpisodeList = async (url) => {
    ...
    console.log("got here wan")
    const $ = cheerio.load(html);
    const eps = []

    $('.mainbox > table').parent('.mainbox').each((_, el) => {
      const title = $(el)?.find("span b")?.text()?.replace(/_/g, " ")?.trim() || "";
      const link = $(el)?.find("span a")?.first()?.attr("href")
        ? this.makeUrl($(el)?.find("span a")?.first()?.attr("href"), this.baseSeriesURL)
        : "";
      const airing = $(el)?.find("span i")?.first()?.text()?.split(":")[1]?.replace(/[(),:_]/g, "").trim() || "";
      const desc = $(el)?.find("small").last().contents()
